#!/usr/bin/env python3
"""Run EXP-001 local-only non-neural sanity baselines."""
...
from thesis_traffic_drift.exp001_training import load_baseline_config, run_baselines, write_results


DEFAULT_CONFIG = Path("configs/EXP-001-baselines.template.yaml")
DEFAULT_INPUT_DIR = Path("artifacts/local/exp-001")
DEFAULT_OUTPUT_DIR = Path("artifacts/local/exp-001/baselines")
...
def ensure_writable_output(output_dir, overwrite):
    if output_dir.exists() and any(output_dir.iterdir()):
        if not overwrite:
            raise SystemExit("ERROR: output-dir is non-empty; pass --overwrite to replace local baseline outputs.")
...
def main() -> int:
    ...
    parser.add_argument("--dry-run", action="store_true", help="Validate planned local baseline run without writing outputs.")
    ...
    planned = ["metrics_summary.json", "metrics_summary.csv"]
    print("EXP-001 local baseline inputs configured.")
    ...
    results = run_baselines(input_dir, input_length=input_length, horizon=horizon, input_label=display_path(input_dir))
    write_results(output_dir, results)
    print("EXP-001 local baseline evaluation complete.")
